...
// названия классов плохо отражают свою сущность
// класс Printable нарушен Принцип разделения интерфейса
// saveTo нарушен принцип открытости/закрытости
// saveTo привязан к потоку ofstream, возможно было бы лучше использовать ostream, на случай если понадобится вывод не в файл
// класс Data нарушает LSP изменяя поведение методов, кроме строки может вернуть исключение